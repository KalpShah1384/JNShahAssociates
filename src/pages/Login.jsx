...
import { Mail, Lock, ArrowRight, ShieldCheck, Fingerprint, AlertCircle, User, Building2 } from 'lucide-react';
            ...
            if (isForgotPassword) {
                ...
                if (resetError) {
                    if (resetError.status === 400) {
                        throw new Error("Redirect URL not authorized. Please add http://localhost:5173/update-password to your Supabase Redirect URLs.");
                    }
                    throw resetError;
                }
            ...
            if (isSignUp) {
                ...
                if (authError) {
                    if (authError.status === 422) {
                        throw new Error("This email is already registered or your password is too weak (Check your Supabase 8-character and symbol requirements).");
                    }
                    throw authError;
                }

                setSuccessMessage("Portal access requested! Check your email for verification.");
                setTimeout(() => {
                    setIsSignUp(false);
                    setSuccessMessage(null);
                }, 4000);
            ...
            >
                <div className="bg-white rounded-[2.5rem] p-10 shadow-2xl shadow-brand-navy/5 border border-brand-ice relative z-10">
                    <div className="flex flex-col items-center mb-10 text-center">
                        <div className="mb-6">
                            <img src={Logo} alt="JN Shah Associates Logo" className="h-16 md:h-20 w-auto object-contain transition-transform hover:scale-105 duration-300" />
                        </div>
                        <h1 style={{ fontFamily: "'Playfair Display', serif" }} className="text-3xl md:text-4xl font-bold text-brand-navy mb-3 tracking-normal">
                        ...
                        <p className="text-brand-navy/60 font-black tracking-widest uppercase text-xs sm:text-sm md:text-base transition-all">
                            {isForgotPassword ? 'Reset Password' : isSignUp ? 'Client Registration' : 'Client Login'}
                        </p>
                    </div>
                    ...
                    <form onSubmit={handleAuth} className="space-y-6">
                        <AnimatePresence mode='wait'>
                            {isSignUp && !isForgotPassword && (
                                ...
                                >
                                    ...
                                    <div className="space-y-2">
                                        <label className="text-xs font-black uppercase tracking-widest text-brand-navy/40 ml-1">Company / Organization</label>
                                        <div className="relative group">
                                            <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                        ...
                        {!isForgotPassword && (
                            <div className="space-y-2">
                                <label className="text-xs font-black uppercase tracking-widest text-brand-navy/40 ml-1">Secure Password</label>
                                <div className="relative group">
                                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                        ...
                        {!isSignUp && !isForgotPassword && (
                            <div className="flex items-center justify-between text-sm py-2">
                                <label className="flex items-center space-x-2 cursor-pointer group">
                                    <input type="checkbox" className="w-4 h-4 rounded border-brand-ice text-brand-primary focus:ring-brand-primary" />
                                    <span className="text-brand-navy/60 font-bold group-hover:text-brand-navy transition-colors">Remember me</span>
                                </label>
                                <button
                                    type="button"
                        ...
                        >
                            {isLoading ? (
                                ...
                                <>
                                    <span>{isForgotPassword ? 'Reset Password' : isSignUp ? 'Create Account' : 'Authenticate'}</span>
                    ...
                    <div className="mt-10 pt-10 border-t border-brand-ice flex flex-col items-center">
                        <div className="flex items-center space-x-2 text-brand-navy/40 mb-6 font-bold text-sm">
                            ...
                            <span>{isSignUp ? 'Already have access?' : 'First time here?'}</span>
                        </div>
                        <button
                            onClick={() => {
