...
const Dashboard = () => {
  ...
  const [isAdminTargetDropdownOpen, setIsAdminTargetDropdownOpen] =
    useState(false);
  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      if (!session) {
        ...
        const isRecovery =
          session.user?.amr?.some((m) => m.method === "recovery") ||
  ...
  const notifications = deadlines.filter(
    (d) => d.task && d.task.startsWith("[NOTIFICATION] "),
  );
  const latestNotificationMessage =
    notifications.length > 0
      ? notifications[0].task.replace("[NOTIFICATION] ", "")
      : "Dashboard ready.";
  const normalDeadlines = deadlines.filter(
    (d) => !d.task || !d.task.startsWith("[NOTIFICATION] "),
  );
  ...
  const handleSendAdminNotification = async () => {
    ...
    const payload = {
      ...
      urgency: "High",
      target_user_id:
        adminNotificationTarget === "ALL" ? null : adminNotificationTarget,
    };
  ...
  const handleReplaceDocument = async (doc, file) => {
    ...
    try {
      ...
      await supabase
        .from("documents")
        .update({ file_url: filePath, name: file.name, status: "In Review" })
        .eq("id", doc.id);
  ...
  const handleAvatarUpload = async (file) => {
    ...
    try {
      ...
      const {
        data: { publicUrl },
      } = supabase.storage.from("client-vault").getPublicUrl(filePath);
  ...
  const stats = isAdmin
    ? [
        {
          label: "Total Clients",
          value: Object.keys(profiles).length.toString().padStart(2, "0"),
          icon: UserCircle,
          color: "brand-navy",
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] pt-16 md:pt-24 pb-12 px-4 sm:px-6 lg:px-8">
      <SEO
        ...
        description={
          isAdmin
            ? "Admin dashboard for JN Shah Associates to manage client compliance, documents, and statutory deadlines securely."
            : "Secure client dashboard for JN Shah Associates with document vault, compliance deadlines, and filing status overview."
        }
      />
        ...
        {activeTab === "overview" ? (
          ...
          <div className="bg-white rounded-[2rem] md:rounded-[2.5rem] border border-brand-ice shadow-2xl shadow-brand-navy/5 overflow-hidden">
            <div className="p-6 md:p-10 border-b border-brand-ice bg-[#fcfdfe]">
              <h2 className="text-xl md:text-2xl font-black text-brand-navy font-heading">
                Client Directory
              </h2>
              <p className="text-[10px] font-bold text-brand-navy/40 uppercase mt-1 tracking-widest">
                Profiles & Compliance Health
              </p>
            </div>
            ...
            <div className="block md:hidden p-4 space-y-4">
              {Object.values(profiles).map((client, idx) => (
                <div
                  key={idx}
                  className="p-6 bg-brand-bg/40 border border-brand-ice rounded-3xl space-y-4"
                >
                  <div className="flex items-center space-x-3">
                    <div className="w-12 h-12 rounded-2xl bg-brand-primary text-white flex items-center justify-center font-black">
                  ...
                  <div className="flex justify-between items-center pt-2 border-t border-brand-ice/50">
                    <div className="flex items-center space-x-3">
                      <button
                        ...
                        className="text-brand-primary font-black text-[10px] uppercase tracking-widest flex items-center"
                      >
                    ...
                    <button
                      ...
                      className="text-brand-primary font-black text-[10px] uppercase tracking-widest"
                    >
                      Open Hub
                    </button>
                  </div>
            ...
            <div className="hidden md:block overflow-x-auto">
              <table className="w-full text-left">
                <thead className="bg-[#fcfdfe] border-b border-brand-ice">
                  <tr>
                    <th className="px-8 py-5 text-[10px] font-black text-brand-navy/40 uppercase tracking-widest text-left">
                      Client Identity
                    </th>
                    <th className="px-8 py-5 text-[10px] font-black text-brand-navy/40 uppercase tracking-widest text-left">
                      Company
                    </th>
                    <th className="px-8 py-5 text-[10px] font-black text-brand-navy/40 uppercase tracking-widest text-center">
                      Status
                    </th>
                    <th className="px-8 py-5 text-[10px] font-black text-brand-navy/40 uppercase tracking-widest text-right">
                      Action
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-brand-ice/50 text-left">
                  {Object.values(profiles).map((client, idx) => {
                    const clientDocs = allDocuments.filter(
                      (d) => d.user_id === client.id,
                    );
                    const needsAction = clientDocs.some(
                      (d) => d.status === "Action Required",
                    );
                    return (
                      <tr
                        key={idx}
                      ...
                      >
                        <td className="px-8 py-6">
                          <div className="flex items-center space-x-4">
                            ...
                            <div>
                              ...
                              <p className="text-[9px] font-bold text-brand-navy/40 tracking-tighter">
                        ...
                        <td className="px-8 py-6 text-center">
                          <span
                            className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${needsAction ? "bg-red-50 text-red-500" : "bg-emerald-50 text-emerald-500"}`}
                          >
                            {needsAction ? "Needs Action" : "Compliant"}
                          </span>
                        </td>
                        <td className="px-8 py-6 text-right">
                          <div className="flex items-center justify-end space-x-2">
                            ...
                            <button
                              onClick={() => setActiveTab("overview")}
                              className="p-2 bg-brand-bg rounded-lg hover:bg-brand-primary hover:text-white transition-all"
                            >
      ...
      <AnimatePresence>
        {showDeadlineModal && (
          <DeadlineModal
            ...
            handleSubmitDeadline={handleSubmitDeadline}
