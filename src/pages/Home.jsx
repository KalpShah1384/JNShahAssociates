  ...
  const fetchNews = async () => {
    try {
      ...
      if (
        ...
        console.log(
          "No specific CA news in 48h, trying broader finance query...",
        );
        const fallbackQuery =
          'finance india OR "business news" OR "economy india"';
      ...
      if (data.status === "success" && data.results) {
        ...
        setNews(
          filtered.length > 0 ? filtered.slice(0, 3) : data.results.slice(0, 3),
        );
      }
    } catch (error) {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 lg:gap-16 items-center">
            ...
            >
              <div className="relative w-56 sm:w-64 md:w-72 lg:w-80">
                ...
                <div className="relative bg-white/90 border border-brand-ice rounded-[2.5rem] shadow-2xl shadow-brand-navy/10 px-8 py-10 flex flex-col items-center text-center overflow-hidden">
                  ...
                  <div className="relative flex items-center justify-center gap-6 mt-2 text-[11px] sm:text-xs font-semibold text-corporate-text/70">
                    <div className="flex flex-col items-center">
                      ...
                      <span className="uppercase tracking-[0.15em] text-[9px]">
                        Clients
                      </span>
                    </div>
                    <div className="w-px h-8 bg-brand-ice" />
                    <div className="flex flex-col items-center">
                      <span className="text-brand-navy font-extrabold text-lg">
                        ICAI
                      </span>
      ...
      <section className="py-20 md:py-24 bg-brand-ice/20 border-y border-brand-ice/30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12 md:mb-16">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold mb-4 text-brand-navy">
              Core Service Excellence
            </h2>
            <p className="text-sm sm:text-base text-corporate-text max-w-2xl mx-auto font-medium">
              Providing a specialized suite of services tailored to meet the
            ...
            {[
              {
                icon: Briefcase,
                title: "Direct Tax",
                desc: "Expert income tax planning, filing, and representation services.",
                slug: "tax",
              },
              {
                icon: FileText,
                title: "GST Solutions",
                desc: "End-to-end GST compliance, audit, and strategic advisory.",
                slug: "gst",
              },
              {
                icon: Shield,
                title: "Audit & Assurance",
                desc: "Robust auditing services ensuring transparency and compliance.",
                slug: "audit",
              },
              {
                icon: TrendingUp,
                title: "Business Advisory",
                desc: "Strategic consulting to optimize your business operations.",
                slug: "advisory",
              },
            ].map((service, index) => (
              <motion.div
              ...
              >
                ...
                <h3 className="text-lg md:text-xl font-bold mb-3 text-brand-navy">
                  {service.title}
                </h3>
                <p className="text-xs sm:text-sm text-corporate-text leading-relaxed mb-6 flex-grow font-medium">
                  {service.desc}
                </p>
                <Link
                  to={`/services/${service.slug}`}
                  className="text-brand-primary flex items-center text-xs sm:text-sm font-bold group-hover:translate-x-1 transition-transform"
                >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row justify-between items-center md:items-end mb-12 md:mb-16 gap-6 text-center md:text-left">
            <div className="flex flex-col items-center md:items-start">
              ...
              <h2 className="text-2xl sm:text-3xl md:text-5xl font-black text-brand-navy font-heading">
                Latest <span className="text-brand-primary">Updates</span>
              </h2>
            </div>
            <Link
              to="/knowledge-hub"
              className="text-brand-navy font-black text-[10px] md:text-xs uppercase tracking-widest border-b-2 border-brand-primary pb-1 hover:text-brand-primary transition-colors"
            >
              View Knowledge Hub
            </Link>
          </div>
            ...
            {loading ? (
              Array(3)
                ...
                .map((_, i) => (
                  <div
                    key={i}
                    className="p-6 md:p-8 bg-brand-bg/40 rounded-[2rem] md:rounded-[2.5rem] border border-brand-ice animate-pulse h-64"
                  >
              ...
              news.map((update, idx) => (
                <div
                  key={idx}
                  className="p-6 md:p-8 bg-brand-bg/40 rounded-[2rem] md:rounded-[2.5rem] border border-brand-ice hover:bg-white hover:shadow-2xl hover:shadow-brand-navy/5 transition-all group flex flex-col h-full"
                >
                  <div className="flex justify-between items-start mb-6">
                    <span className="text-[10px] font-black text-brand-navy/40 uppercase tracking-widest">
                      {update.pubDate
                        ? new Date(update.pubDate).toLocaleDateString("en-IN", {
                            day: "2-digit",
                            month: "short",
                          })
                        : "UPDATE"}
                    </span>
                    <span className="bg-brand-primary/10 text-brand-primary px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-wider">
                      Update
                    </span>
                  </div>
                  ...
                  <p className="text-xs sm:text-sm text-corporate-text/70 leading-relaxed font-medium line-clamp-3 mb-6 flex-grow">
                    {update.description ||
                      "Statutory regulatory update for businesses and professionals."}
                  </p>
                  <a
                    href={update.link}
                    target="_blank"
              ...
              <div className="col-span-full py-20 text-center">
                <p className="text-brand-navy font-bold uppercase tracking-widest text-xs opacity-40">
                  Connecting to ICAI updates...
                </p>
              </div>
            )}
          </div>
      ...
      <section className="py-24 md:py-32 bg-brand-navy relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="text-center mb-16 md:mb-24">
            <h2 className="text-2xl sm:text-3xl md:text-5xl font-black text-white mb-6 font-heading">
              Sectors We{" "}
              <span className="text-brand-primary italic">Serve</span>
            </h2>
            <p className="text-sm sm:text-base text-brand-ice/60 max-w-2xl mx-auto font-medium">
              Domain expertise tailored for specialized industrial requirements.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 md:gap-6">
            {[
              ...
              >
                ...
                <h4 className="text-white font-black text-[9px] md:text-[11px] uppercase tracking-widest">
                  {sector.name}
                </h4>
              </motion.div>
            ))}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-16 items-center">
            <div>
              ...
              <p className="text-base sm:text-lg text-corporate-text mb-8 md:mb-12 flex items-start font-medium">
                ...
                commitment. We don't just provide services; we build long-term
                ...
                {[
                  {
                    ...
                    desc: "Years of deep domain experience in tax laws and audit standards.",
                  },
                  {
                    title: "Timely Compliance",
                    desc: "Rigorous systems to ensure all your deadlines are met perfectly.",
                  },
                  {
                    title: "Confidentiality",
                    desc: "Utmost security and privacy of your sensitive financial data.",
                  },
                  {
                    title: "Industry Specific Focus",
                    desc: "Customized solutions for Startups, IT, Manufacturing, and SMEs.",
                  },
                ].map((item, index) => (
                  <div key={index} className="flex items-start space-x-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs md:text-sm text-corporate-text font-medium">
                        {item.desc}
                      </p>
                    </div>
                  </div>
              ...
              <div className="bg-brand-ice/20 border border-brand-ice rounded-3xl aspect-square flex items-center justify-center relative max-w-lg mx-auto">
                ...
                <div className="grid grid-cols-2 gap-4 md:gap-6 relative z-10 w-full px-4 md:px-8">
                  <div className="bg-white p-4 md:p-6 rounded-2xl text-center shadow-sm border border-brand-ice/50">
                    ...
                    <div className="text-corporate-text text-[9px] md:text-xs font-bold uppercase tracking-wider">
                      Years of Exp.
                    </div>
                  </div>
                  <div className="bg-white p-4 md:p-6 rounded-2xl text-center shadow-sm border border-brand-ice/50">
      ...
      <section className="py-20 md:py-24 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="bg-brand-navy rounded-[2rem] md:rounded-3xl p-8 md:p-12 text-center relative overflow-hidden shadow-2xl shadow-brand-navy/20">
            ...
            <h2 className="text-2xl sm:text-3xl md:text-5xl font-bold mb-6 text-white tracking-tight">
              Ready for Financial Growth?
            </h2>
            <p className="text-base md:text-xl text-white/80 mb-8 md:mb-10 max-w-2xl mx-auto leading-relaxed font-medium">
              Schedule a free consultation today and let our experts guide your
              business towards total compliance and fiscal efficiency.
            </p>
            <Link
              to="/book"
              className="bg-white text-brand-navy px-8 md:px-12 py-3 md:py-4 rounded-xl font-bold text-base md:text-lg hover:bg-brand-ice transition-all shadow-xl active:scale-95 inline-block"
            >
              Schedule Now
            </Link>
          </div>
